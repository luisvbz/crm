        ...
        <div class="p-6">
            ...
            <Form @submit="submit" :initial-values="initialValues" class="space-y-5">
                ...
                <div class="mt-8 flex justify-end gap-3 pt-4 border-t border-neutral-100">
                    <button type="button" class="btn btn-secondary" :disabled="inertiaForm.processing"
                        @click="closeModal">Cancelar</button>
                    <button type="submit" class="btn btn-primary flex items-center gap-2"
                        :disabled="inertiaForm.processing">
                        <svg v-if="inertiaForm.processing" class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                            </circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                        </svg>
                        {{ inertiaForm.processing ? 'Guardando...' : (lead ? 'Guardar Cambios' : 'Crear Lead') }}
                    </button>
                </div>
            </Form>
        </div>
